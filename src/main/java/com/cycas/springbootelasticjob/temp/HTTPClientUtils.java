...
//		}
	 ...
	 * @throws Exception
	 */
	public static String sendMapHttpToBaiduInstallment(String url, Map<String, String> data,
									 String charset,String sign) throws Exception {
		...
		List<NameValuePair>   list =  data.entrySet().stream().map(e -> new BasicNameValuePair(e.getKey(), e.getValue())).collect(Collectors.toList());
		httpPost.setHeader("channel",data.get("channel"));
		...
		if (HttpStatus.SC_OK != status.getStatusCode()) {
			logger.error("Http请求异常,httpcode=" + status.getStatusCode());
			throw new Exception("Http请求异常,httpcode=" + status.getStatusCode());
		}
		HttpEntity entity = response.getEntity();
		logger.info("共耗时：" + ((System.currentTimeMillis() - old) / 1000.0) + "秒");  //转化用时
		return EntityUtils.toString(entity, charset);
	}

	/**
	 * 设置请求头，发起POST请求，JSON格式
	 * @param httpUrl
	 * @param params
	 * @param headerMap
	 * @param encoding
	 * @return
	 */
	public static String sendHttpPost(String httpUrl, String params, Map<String, String> headerMap, String encoding) {
	...
	public static String sendMapHttp(String url, Map<String, String> data,
...
//		requestConfigBuilder.setSocketTimeout(30000).setMaxRedirects(1);
		httpPost.setConfig(requestConfigBuilder.build());
		List<NameValuePair>   list =  data.entrySet().stream().map(e -> new BasicNameValuePair(e.getKey(), e.getValue())).collect(Collectors.toList());
		// url格式编码
		UrlEncodedFormEntity uefEntity = new UrlEncodedFormEntity(list, "UTF-8");
	...
	public static String sendStrHttp(String url, String data,String charset) throws Exception {
		HttpClient httpClient = HttpClientBuilder.create().build();
		HttpPost httpPost = new HttpPost(url);
		RequestConfig.Builder requestConfigBuilder = RequestConfig.custom();
		requestConfigBuilder.setConnectionRequestTimeout(1000).setMaxRedirects(1);
